...
def repo_slug():
    """owner/repo for the "Report a problem" GitHub issue link (P2-0060), read from the git
    remote at build time so quiz.html — gitignored, regenerated by this script — never
    hardcodes it. Returns "" when there's no `origin` yet (true as of this writing: see
    PUBLISHING.md, `git remote -v` is empty until gh-pages is set up) or the URL doesn't
    parse to a github.com owner/repo. The JS hides the report button whenever REPO is empty
    rather than link to a broken URL."""
    try:
        r = subprocess.run(["git", "remote", "get-url", "origin"], cwd=DST,
...
/* Report-a-problem dialog (P2-0060): a variant of the confirm dialog above, not a second
   dialog system — same .confirm-mask/.confirm-box, plus form-field styling. No <form>
   element (see the JS comment by doLookup — a sandboxed frame may lack allow-forms) and no
   window.open() (see the comment by askConfirm — silently a no-op under the same sandbox):
   the actual GitHub navigation is a real <a target="_blank"> click, same pattern as the
   citation links in render(). */
.report-box{max-width:26rem;text-align:left}
.report-box h3{margin:0 0 .7rem;font-size:1rem}
.report-box .report-qid{margin:0 0 .8rem;font-family:var(--mono);font-size:.8rem;opacity:.75}
.report-box label{display:block;font-size:.8rem;color:var(--muted);margin:.7rem 0 .25rem}
.report-box label:first-of-type{margin-top:0}
.report-select,.report-note{font:inherit;color:inherit;background:var(--card);width:100%;
  border:1px solid var(--line);border-radius:.5rem;padding:.5rem .6rem;box-sizing:border-box}
.report-note{resize:vertical;font-family:inherit}
.report-select:focus-visible,.report-note:focus-visible{outline:2px solid var(--accent);outline-offset:2px}
  ...
  <section id="quiz" class="hidden">
    ...
    <div class="qcard">
      ...
      <div class="nav">
        ...
        <div class="spacer"></div>
        <button class="btn" id="reportBtn" type="button">Prijavi problem</button>
...
<div id="reportMask" class="confirm-mask hidden">
  <div class="confirm-box report-box" role="alertdialog" aria-modal="true" aria-labelledby="reportH">
    <h3 id="reportH">Prijavi problem</h3>
    <p class="report-qid" id="reportQid"></p>
    <label for="reportReason">Razlog</label>
    <select id="reportReason" class="report-select">
      <option>Netočan odgovor</option>
      <option>Loša ili pogrešna citacija</option>
      <option>Tipfeler ili nejasan tekst</option>
      <option>Nedostaje ili pogrešna slika</option>
      <option>Ostalo</option>
    </select>
    <label for="reportNote">Opis (nije obavezno)</label>
    <textarea id="reportNote" class="report-note" rows="4" placeholder="Detalji, ako ih imaš…"></textarea>
    <div class="nav">
      <a id="reportGo" class="btn primary" href="#" target="_blank" rel="noopener noreferrer">Otvori GitHub issue</a>
      <button id="reportCancel" class="btn" type="button">Zatvori</button>
    </div>
  </div>
</div>
...
// owner/repo for the report-a-problem GitHub issue link (P2-0060), baked in at build time
// by build_quiz.py's repo_slug() from `git remote get-url origin` — never hardcoded here.
// "" whenever the build had no origin to read; see the JS below by REPORT_ENABLED.
const REPO = __REPO__;
const SEC = {
...
/* ---------- report a problem (P2-0060) ----------
   The quiz is static with no backend of its own — GitHub, via a prefilled issues/new URL, is
   the only thing already available to receive a report. Built the same way as the citation
   links in render() (a real <a target="_blank">, not window.open() — see the comment by
   askConfirm for why that would silently do nothing here) so it survives the sandboxed
   iframe this page also runs in on claude.ai. queue/from_issues.py is the other half: it
   turns each open `quiz-feedback`-labelled issue into a queue/todo/ item at the start of a
   drain. */
let reportPrev = null;
function reportUrl(){
  const q = S.items[S.i].q;
  const reason = $("#reportReason").value;
  const note = $("#reportNote").value.trim();
  const cites = q.cs ? [q.cs.src + " — " + q.cs.loc] : [];
  (q.c || []).forEach(c => cites.push(c.src + " — " + c.loc));
  const title = "[quiz] " + q.id + ": " + reason;
  const body = [
    "Pitanje: " + q.id,
    "Razlog: " + reason,
    "Trenutni odgovor u kvizu: " + (q.o[q.a] || ""),
    cites.length ? "Citacija: " + cites.join("; ") : "",
    "",
    "Opis:",
    note || "(nije upisan)",
  ].join("\n");
  return "https://github.com/" + REPO + "/issues/new?title=" + encodeURIComponent(title) +
         "&body=" + encodeURIComponent(body) + "&labels=" + encodeURIComponent("quiz-feedback");
}
function openReport(){
  const q = S.items[S.i].q;
  $("#reportQid").textContent = q.id;
  $("#reportReason").value = "Netočan odgovor";
  $("#reportNote").value = "";
  $("#reportMask").classList.remove("hidden");
  reportPrev = document.activeElement;
  $("#reportReason").focus();
  const onKey = e => { if(e.key === "Escape"){ e.preventDefault(); done(); } };
  function done(){
    $("#reportMask").classList.add("hidden");
    document.removeEventListener("keydown", onKey, true);
    if(reportPrev && reportPrev.focus) reportPrev.focus();
  }
  document.addEventListener("keydown", onKey, true);
  $("#reportCancel").onclick = done;
  $("#reportMask").onclick = e => { if(e.target === $("#reportMask")) done(); };
  // Computed just-in-time on click, the same tick the browser reads .href to navigate —
  // reason/note can still change after the dialog opens, so the URL can't be built once
  // up front the way #reportQid is.
  $("#reportGo").onclick = () => { $("#reportGo").href = reportUrl(); };
}
// REPO is "" whenever the build had no `origin` to read (true as of this writing — see
// PUBLISHING.md); hide the control rather than wire up a link to a broken URL.
...
out = DST / "quiz.html"
html = HTML.replace("__PAYLOAD__", payload).replace("__REPO__", json.dumps(REPO))
out.write_text(html, encoding="utf-8")
print(f"wrote {out}  ({out.stat().st_size//1024} KB)  questions={len(live)}"
      + (f"  repo={REPO}" if REPO else "  repo=(none — report button hidden)"))
